...
const EditBlogPost = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch post
                const postResponse = await Axios.get(`/api/blog/get/${postSlug}`);
                ...
                if (postResponse.data.success) {
                    ...
                    setViewImageURL(post.featuredImage);
                }
                ...
                if (!allCategories || allCategories.length === 0) {
                    ...
                    if (categoriesResponse.data.success) {
                        dispatch({ type: 'blog/setAllCategories', payload: categoriesResponse.data.data });
                    }
                }
            } catch (error) {
        ...
        fetchData();
    }, [postSlug, dispatch, allCategories, navigate]);
    ...
    const handleSubmit = async (e) => {
        ...
        const postData = {
            ...formData,
            categories: formData.categories.map(catId => ({ _id: catId })),
            tags: formData.tags.filter(tag => tag.trim() !== ''),
            slug: postSlug // Include the slug in the request
        };
        
        try {
            setLoading(true);
            const response = await Axios({
                ...SummaryApi.updateBlog,
                data: postData
            });
